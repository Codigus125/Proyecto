...
function FormularioAsistente_6() {
	...
	const UPDATE_ASISTENTE_F6 =gql`
	mutation Mutation($rut: String!, $nuevosDatos: Formulario6!) {
		updateAsistente_F6(rut: $rut, NuevosDatos: $nuevosDatos) {
		  martes {
			Ma_primero
			Ma_segundo
			Ma_tercero
	...
	const [handleUpdate] = useMutation(UPDATE_ASISTENTE_F6,{variables:{rut:Rut ,nuevosDatos:NuevosDatos }});

	const handleUpdateAsistenteF6 = async (e) => {
		e.preventDefault();
		try {
			...
			<div className="container pt-3 pb-2 mt-2 text-bg border border-2 border-dark rounded col-12">
				...
				<form ref={formRef} onSubmit={handleSubmit}>
					...
					<div className="btn-group-wide d-grip mb-2 text-center">
						...
						onClick={handleUpdateAsistenteF6}>Guardar</button>
					</div>
				</form>
				<div></div>
			</div>
...
export default FormularioAsistente_6;